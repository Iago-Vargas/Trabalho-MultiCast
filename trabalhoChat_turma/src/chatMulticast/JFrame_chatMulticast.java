...
public class JFrame_chatMulticast extends javax.swing.JFrame {
    ...
    private void addUsuarioNaLista(String nick) {
        ...
        for (int i = 0; i < usuariosModel.size(); i++) {
            if (usuariosModel.get(i).equals(nick)) { existe = true; break; }
        }
        if (!existe) usuariosModel.addElement(nick);
    }

    private void removerUsuarioDaLista(String nick) {
        for (int i = 0; i < usuariosModel.size(); i++) {
            if (usuariosModel.get(i).equals(nick)) {
    ...
    private void enviarMsg() throws IOException, NumberFormatException, NullPointerException {
        ...
        if (selecionado == null || "Todos".equals(selecionado)) {
            ...
            DatagramPacket pacote = ComunicadorUDP.montaMensagem(payload, jTextField_GrupoIP.getText(), Integer.parseInt(jTextField_Porta.getText()));
            socket.send(pacote);
        } else {
            // Unicast para o usuário selecionado
            InetSocketAddress destino = peers.get(selecionado);
            if (destino == null) {
                JOptionPane.showMessageDialog(this, "Usuário offline ou não resolvido: " + selecionado);
            } else {
                String payload = "PRIV|" + meuNick() + "|" + selecionado + "|" + texto;
                ...
                lista.add("[para " + selecionado + "] " + meuNick() + ": " + texto);
                renderizarMensagens();
            }
        }
